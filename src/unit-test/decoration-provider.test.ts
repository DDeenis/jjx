...
import { asRealPath } from "../workspace-paths";
...
function refresh(
  ...
  provider.onRefresh(
    asRealPath(repositoryRoot),
    new Map([["@", options.statuses ?? []]]),
    new Set((options.tracked ?? []).map((file) => normalizePath(asRealPath(file)))),
    new Map([["@", new Set((options.conflicts ?? []).map((file) => normalizePath(asRealPath(file))))]]),
    options.untracked ?? [],
  );
}

function status(path: string, type: FileStatus["type"] = "M"): FileStatus {
  return { type, path: asRealPath(path), file: path };
}
...
describe("JJDecorationProvider regressions", () => {
  ...
  it("invalidates ignored decorations when a repository is removed", () => {
    ...
    provider.removeStaleRepositories([asRealPath(repoA)]);
